...
namespace ray_test
{
    ...
    {        
         ...
         * 3 layers with 2 nodes each, forward and backbard propagation
         * for one run
         * example from here https://mattmazur.com/2015/03/17/a-step-by-step-backpropagation-example/
         */
        [TestMethod]
        public void ForwardSixNodes()
        {
            ...
            //Forward Propagatiom
            nodeLayerOneOne.ForwardValue(0.05);
            nodeLayerOneTwo.ForwardValue(0.1);

            Assert.AreEqual(0.751, nodeLayerThreeOne.finalValue, 0.01);
            Assert.AreEqual(0.773, nodeLayerThreeTwo.finalValue, 0.01);

            var target_o1 = 0.01;
            var target_o2 = 0.99; 

            // //Backward Propagation
            var e1 = nodeLayerThreeOne.finalValue - target_o1;
            nodeLayerThreeOne.Backpropagate(e1);

            var e2 = nodeLayerThreeTwo.finalValue - target_o2;
            nodeLayerThreeTwo.Backpropagate(e2);


            Assert.AreEqual(0.3589, connectorLayerTwoThreeNodeOneOne.weight, 0.001);
            Assert.AreEqual(0.4087, connectorLayerTwoThreeNodeOneTwo.weight, 0.001);
            Assert.AreEqual(0.5113, connectorLayerTwoThreeNodeTwoOne.weight, 0.001);
            Assert.AreEqual(0.5613, connectorLayerTwoThreeNodeTwoTwo.weight, 0.001);

            Assert.AreEqual(0.16, nodeLayerTwoOne.errorBackProp, 0.01);
            // Assert.AreEqual(0.34, nodeLayerTwoTwo.errorBackProp, 0.01);

            Assert.AreEqual(0.1498, connectorLayerOneTwoNodeOneOne.weight, 0.001);
            Assert.AreEqual(0.1996, connectorLayerOneTwoNodeOneTwo.weight, 0.001);
            Assert.AreEqual(0.2498, connectorLayerOneTwoNodeTwoOne.weight, 0.001);
            Assert.AreEqual(0.2995, connectorLayerOneTwoNodeTwoTwo.weight, 0.001);
        }
    }
}
